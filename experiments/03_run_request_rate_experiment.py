"""Task 3 - Run request-rate strategy autoscaling experiment (default 30 minutes)."""
...
class RequestRateExperimentRunner:
    ...
    def _collect_sample(self) -> Dict[str, Any]:
        ...
        requests_per_target = _parse_latest_datapoint(request_count_resp, "Average")
        ...
        return {
            ...
            "request_count_per_target_avg_1m": requests_per_target,
        }
    ...
    def run(self) -> Dict[str, Any]:
        print(f"\n{'=' * 70}")
        print(f"Request-Rate Strategy Experiment Starting")
        print(f"{'=' * 70}")
        ...
        req_pt = [
            float(sample["request_count_per_target_avg_1m"])
            for sample in self.metric_samples
            if sample.get("request_count_per_target_avg_1m") is not None
        ]
        ...
        return {
            ...
            "scaling_summary": {
                ...
                "avg_request_count_per_target": statistics.mean(req_pt)
                if req_pt
                else 0.0,
                "max_request_count_per_target": max(req_pt) if req_pt else 0.0,
            },
...
def main() -> int:
    ...
    output_path = RESULTS_DIR / "request_rate_experiment_metrics.json"
